# ⚡ 快速测试指南

## 🚀 5分钟快速测试

### 方法1：前端界面测试（最简单）

1. **启动开发服务器**
   ```bash
   npm run dev
   ```

2. **访问市场详情页面**
   ```
   http://localhost:3000/market/[marketId]
   ```

3. **检查提取按钮**
   - 市场解析后应该看到"提取奖励"区域
   - 显示持仓和可提取金额

4. **点击提取**
   - 点击"提取奖励"按钮
   - 确认钱包交易
   - 检查 USDC 余额增加

---

### 方法2：命令行测试（推荐）

1. **设置测试账户私钥**
   ```bash
   export TEST_PRIVATE_KEY=your_private_key_here
   ```

2. **运行测试脚本**
   ```bash
   npx tsx scripts/test-redeem.ts <conditionId> <outcomeIndex>
   ```
   
   示例：
   ```bash
   npx tsx scripts/test-redeem.ts 0x123... 1
   ```

3. **查看结果**
   - 脚本会自动检查所有条件
   - 显示提取前后的 USDC 余额
   - 验证提取金额是否正确

---

## 📋 测试前检查清单

- [ ] 市场已解析（status === 'resolved'）
- [ ] 市场有 condition_id
- [ ] 用户有持仓（买入过 YES/NO）
- [ ] 钱包已连接
- [ ] 有足够的 MATIC 支付 Gas

---

## 🔍 快速验证

### 检查市场状态

```typescript
// 在浏览器控制台运行
const conditionId = '0x...'; // 你的 conditionId
const provider = new ethers.providers.Web3Provider(window.ethereum);
const ctf = new ethers.Contract(
  '0xb171BBc6b1476ee1b6aD4Ac2cA7ed4AfFdFa10a2',
  ['function isResolved(bytes32) view returns (bool)'],
  provider
);
const resolved = await ctf.isResolved(conditionId);
console.log('市场已解析:', resolved);
```

### 检查用户持仓

```typescript
// 在浏览器控制台运行
import { checkRedeemableBalance } from '@/lib/ctf/redeem';

const provider = new ethers.providers.Web3Provider(window.ethereum);
const signer = provider.getSigner();
const userAddress = await signer.getAddress();

const balance = await checkRedeemableBalance(
  provider,
  userAddress,
  conditionId,
  1 // YES
);
console.log('可赎回余额:', balance);
```

---

## ✅ 测试成功标志

- ✅ 提取按钮显示
- ✅ 显示正确的持仓和可提取金额
- ✅ 交易成功确认
- ✅ USDC 余额增加
- ✅ 提取后按钮状态更新

---

## 🐛 常见问题

**Q: 提取按钮不显示？**
A: 检查市场是否已解析，是否有 condition_id

**Q: 显示"无可赎回余额"？**
A: 检查用户是否有持仓（需要先买入 YES/NO）

**Q: 提取失败？**
A: 检查 Gas 是否充足，网络是否正常

---

详细测试指南请查看：[CTF资金托管测试指南.md](./CTF资金托管测试指南.md)





