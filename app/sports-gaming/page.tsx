...
const SportsGamingPage = () => {
  ...
  const { markets: sportsGamingMarkets, loading, error } = useMarketsByCategory('sports-gaming');

  // 🔥 使用 WebSocket 获取实时价格
  const marketIds = sportsGamingMarkets.map(m => m.id);
  ...
  const categories = [
    ...
    { id: 'football', name: '足球' },
    { id: 'esports', name: '电竞' },
    { id: 'international-esports', name: '国际电竞' },
    { id: 'sea-esports', name: '东南亚电竞' },
    { id: 'volleyball', name: '排球' },
    { id: 'athletics', name: '田径' },
    { id: 'multi-sports', name: '综合赛事' }
  ];

  // 先按分类筛选
  let filteredMarkets = selectedCategory === 'all' 
    ? sportsGamingMarkets 
    : sportsGamingMarkets.filter(market => 
        (selectedCategory === 'basketball' && market.category === '篮球') ||
        (selectedCategory === 'football' && market.category === '足球') ||
        (selectedCategory === 'esports' && market.category === '电竞') ||
        (selectedCategory === 'international-esports' && market.category === '国际电竞') ||
        (selectedCategory === 'sea-esports' && market.category === '东南亚电竞') ||
        (selectedCategory === 'volleyball' && market.category === '排球') ||
        (selectedCategory === 'athletics' && market.category === '田径') ||
        (selectedCategory === 'multi-sports' && market.category === '综合赛事')
      );
    ...
    return (
      <div className="min-h-screen bg-gray-50 text-gray-900 flex items-center justify-center">
        <div className="text-center">
          <FontAwesomeIcon icon={faTrophy} className="text-6xl text-purple-600 mb-4 animate-pulse" />
          <p className="text-xl text-gray-600">加载中...</p>
          <p className="text-sm text-gray-500 mt-2">正在获取最新的体育与游戏市场数据</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 text-gray-900 flex items-center justify-center">
        <div className="text-center">
          <FontAwesomeIcon icon={faTrophy} className="text-6xl text-red-400 mb-4" />
        ...
        <div className="flex flex-col lg:flex-row gap-4 mb-6">
          ...
          <div className="relative w-full lg:w-64">
            <input
              ...
              className="w-full bg-white border border-gray-300 rounded-lg px-4 py-2 pl-10 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-green-400 transition-colors"
            />
            <svg
...
export default SportsGamingPage;
